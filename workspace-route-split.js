(() => {
  ...
  function labelBlogButton(button) {
    if (!button || button.dataset.routeSplitLabel === 'blog') return;
    button.dataset.routeSplitLabel = 'blog';
  ...
  function injectSettingsNavigation() {
    ...
    if (top && !top.querySelector('[data-settings-route]')) {
      top.insertBefore(
        makeSettingsButton('ghost-button settings-top-button'),
        top.querySelector('#logout-button')
      );
    }
    if (side && !side.querySelector('[data-settings-route]')) {
      side.insertBefore(
        makeSettingsButton('nav-button settings-nav-button'),
        side.querySelector('.sidebar-divider')
      );
    }
  ...
  function setRouteClasses(route) {
    document.body.classList.toggle(
      'route-blog',
      route === BLOG_ROUTE || route === LEGACY_BLOG_ROUTE
    );
  ...
  function setActiveNavigation(route) {
    if (route === BLOG_ROUTE || route === LEGACY_BLOG_ROUTE) {
      document.querySelectorAll('[data-route], [data-settings-route]')
        .forEach((button) => button.classList.remove('active'));
      document.querySelectorAll('[data-admin-route="admin"]')
        .forEach((button) => button.classList.add('active'));
      return;
    }

    if (route === SETTINGS_ROUTE) {
      document.querySelectorAll('[data-route], [data-admin-route]')
        .forEach((button) => button.classList.remove('active'));
      document.querySelectorAll('[data-settings-route]')
        .forEach((button) => button.classList.add('active'));
      return;
    }

    document.querySelectorAll('[data-settings-route], [data-admin-route]')
      .forEach((button) => button.classList.remove('active'));
  }
  ...
  function polishBlogPage() {
    ...
    if (page.dataset.blogPolished !== 'true') {
      page.dataset.blogPolished = 'true';
      ...
      if (hero) {
        ...
        if (description) {
          description.textContent = '게시 일정, 주제 기획, 초안, 제품 조사와 검증 링크를 한곳에서 관리합니다.';
        }
      ...
      if (toolsTitle?.textContent.trim() === '관리 데이터') {
        toolsTitle.textContent = '블로그 데이터';
      }
    }
  ...
  function finalizeBlogRoute(force = false) {
    ...
    if (!force && !blogReady() && Date.now() - blogStartedAt < 4500) {
      setTimeout(scheduleSync, 140);
      return;
    }
  ...
  function settingsMarkup() {
    return `
      <section class="admin-page settings-page" data-settings-rendered="true">
  ...
  function refreshSettingsPanels(runId) {
    ...
    if (
      !document.getElementById('ops-admin-suite') &&
      window.BLUEBLACK_CONTENT_OPERATIONS?.refreshAdmin
    ) {
    ...
    if (
      document.getElementById('ops-admin-suite') ||
      document.getElementById('trusted-device-manager')
    ) {
  ...
  function renderSettings() {
    ...
    let page = main.querySelector('.settings-page');
    if (!page) {
      main.innerHTML = settingsMarkup();
      page = main.querySelector('.settings-page');

      const guide = page?.querySelector('#settings-open-guide');
  ...
  function sync() {
    ...
    if (route === LEGACY_BLOG_ROUTE || route === BLOG_ROUTE) {
      beginBlogRoute();
    } else if (route === SETTINGS_ROUTE) {
      renderSettings();
    } else {
      blogStartedAt = 0;
      ...
      if (document.querySelector('.workspace')) {
        document.title = 'BlueBlack SNS — Internal Workspace';
      }
    }
  }
